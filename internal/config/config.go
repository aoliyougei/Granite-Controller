...
func Load(path string) (Config, error) {
	...
	threads, err := envInt("GRANITE_THREADS", raw.Granite.Threads)
	if err != nil {
		return Config{}, err
	}
	contextSize, err := envInt("GRANITE_CONTEXT_SIZE", raw.Granite.ContextSize)
	if err != nil {
		return Config{}, err
	}
	maxTokens, err := envInt("GRANITE_MAX_TOKENS", raw.Granite.MaxTokens)
	if err != nil {
		return Config{}, err
	}
	maxMessage, err := envInt("GRANITE_MAX_MESSAGE_LENGTH", raw.Granite.MaxMessageLength)
	if err != nil {
		return Config{}, err
	}
	startup, err := envDuration("GRANITE_STARTUP_TIMEOUT", raw.Granite.StartupTimeout)
	if err != nil {
		return Config{}, err
	}
	request, err := envDuration("GRANITE_REQUEST_TIMEOUT", raw.Granite.RequestTimeout)
	if err != nil {
		return Config{}, err
	}
	dedup, err := envDuration("GRANITE_ACTION_DEDUP_WINDOW", raw.Granite.ActionDedupWindow)
	if err != nil {
		return Config{}, err
	}
	upstream, err := envDuration("INFRA_CONTROL_TIMEOUT", raw.InfraControl.Timeout)
	if err != nil {
		return Config{}, err
	}
	allowStop, err := envBool("GRANITE_ALLOW_FORCE_STOP", raw.Granite.AllowForceStop)
	if err != nil {
		return Config{}, err
	}
	cfg := Config{
