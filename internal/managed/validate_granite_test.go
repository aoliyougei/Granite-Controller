...
func TestValidateToolCall(t *testing.T) {
	tests := []struct{ text, name, args string; allow bool; want Action; code string }{
	...
	for _, tc := range tests { t.Run(tc.text+tc.name, func(t *testing.T) {
		in, err := AnalyzeInput(tc.text); if err != nil { t.Fatal(err) }
		got, apiErr := ValidateToolCall(in, granite.ToolCall{Name:tc.name, Arguments:tc.args}, tc.allow)
		if tc.code == "" { if apiErr != nil || got.Action != tc.want || got.VMID != 3052 { t.Fatalf("got=%+v err=%v", got, apiErr) } } else if apiErr == nil || apiErr.Code != tc.code { t.Fatalf("err=%v", apiErr) }
	}) }
}
