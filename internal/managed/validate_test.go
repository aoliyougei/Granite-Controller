...
func validEnvelope() native.Envelope {
	return native.Envelope{Type: "call", Success: true, Confidence: floatPointer(0.92), Validation: native.Validation{Ungrounded: []string{}, Negation: false}, FunctionCalls: []native.FunctionCall{{Name: "pve_vm_start", Arguments: map[string]json.RawMessage{"vmid": json.RawMessage(`3052`)}}}}
}
...
func TestValidateCallRejectsUnsafeNeedleResults(t *testing.T) {
	tests := []struct {
		...
		}, "managed_tool_validation_failed"}, {"mismatch", func(e *native.Envelope) { e.FunctionCalls[0].Arguments["vmid"] = json.RawMessage(`3053`) }, "argument_mismatch"}, {"missing confidence", func(e *native.Envelope) { e.Confidence = nil }, "low_confidence"}, {"nan", func(e *native.Envelope) { e.Confidence = floatPointer(math.NaN()) }, "low_confidence"}, {"inf", func(e *native.Envelope) { e.Confidence = floatPointer(math.Inf(1)) }, "low_confidence"}, {"out range", func(e *native.Envelope) { e.Confidence = floatPointer(1.1) }, "low_confidence"}, {"below", func(e *native.Envelope) { e.Confidence = floatPointer(0.59) }, "low_confidence"}, {"ungrounded", func(e *native.Envelope) { e.Validation.Ungrounded = []string{"pve_vm_start.vmid"} }, "ungrounded_arguments"}, {"negation", func(e *native.Envelope) { e.Validation.Negation = true }, "managed_tool_validation_failed"}}
