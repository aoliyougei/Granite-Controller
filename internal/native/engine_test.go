...
func (f *fakeABI) Complete(text []byte, tokens int, output []byte) int {
	...
	if len(f.codes) > 0 { code:=f.codes[0]; f.codes=f.codes[1:]; if code<0{return code} }
	if len(f.responses)==0{return -9}
	data:=f.responses[0];f.responses=f.responses[1:]
	copy(output,data)
	return 0
}
...
func TestEngineExecutesResetInitAndAllTurns(t *testing.T){
	...
	first[len(first)-4]=0; first=first[:len(first)-3]
	second:=[]byte(`{"type":"call","success":true,"function_calls":[{"name":"pve_vm_start","arguments":{"vmid":3052}}],"confidence":0.92,"validation":{"ungrounded":[],"negation":false}}`);second=append(second,0)
	...
	if strings.Join(abi.calls,"|")!=strings.Join(want,"|"){t.Fatalf("calls=%v",abi.calls)}
	if string(abi.initArgs[0])!="date: now"||string(abi.initArgs[2])!="/tmp/index"{t.Fatalf("init args=%q",abi.initArgs)}
	if len(got.FunctionCalls)!=1||got.FunctionCalls[0].Name!="pve_vm_start"||got.Confidence==nil||*got.Confidence!=0.92{t.Fatalf("envelope=%+v",got)}
}

func TestEngineRejectsNativeFailuresAndUnsafeOutput(t *testing.T){
	tests:=[]struct{name string; abi *fakeABI; code string; buffer int}{
	...
	for _,tc:=range tests{t.Run(tc.name,func(t *testing.T){engine:=NewEngine(tc.abi,tc.buffer);_,err:=engine.Execute(Request{ToolsJSON:[]byte(`[]`),ToolNames:[]string{"allowed"},MaxNewTokens:1,Turns:[]Turn{{Kind:TurnUser,Text:"x"}}});var nativeErr *NativeError;if !errors.As(err,&nativeErr)||nativeErr.Code!=tc.code{t.Fatalf("err=%+v calls=%v",err,tc.abi.calls)};if tc.name!="init"&&tc.abi.calls[len(tc.abi.calls)-1]!="reset"{t.Fatalf("failure did not reset: %v",tc.abi.calls)}})}
}
...
func TestEngineClearsBufferBetweenCalls(t *testing.T){
	...
	if _,err:=engine.Execute(request);err!=nil{t.Fatal(err)}
	if _,err:=engine.Execute(request);err!=nil{t.Fatal(err)}
}
