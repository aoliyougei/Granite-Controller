...
type Executor interface {
	Execute(Request) (Envelope, error)
}
...
func (d *Dispatcher) loop(factory ExecutorFactory, probe Request) {
	...
	if err != nil {
		d.fail("native engine construction failed")
		return
	}
	if _, err = executor.Execute(probe); err != nil {
		d.fail("native engine initialization probe failed")
		return
	}
	...
	for {
		...
		case task := <-d.queue:
			if err := task.ctx.Err(); err != nil {
				task.response <- result{err: err}
				continue
			}
			envelope, err := executor.Execute(task.request)
			if isFatal(err) {
				d.fail("native engine entered a failed state")
			}
...
func (d *Dispatcher) setState(state State, failure string) {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.state = state
	d.failure = failure
...
func isFatal(err error) bool {
	...
	return errors.As(err, &nativeErr) && nativeErr.Fatal
}
