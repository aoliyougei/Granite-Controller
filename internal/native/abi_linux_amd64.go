...
func cString(value []byte)(*C.char,func(),bool){
	if bytes.IndexByte(value,0)>=0{return nil,func(){},false}
...
func (a *cgoABI) Init(system,tools,index []byte)int{
	cs,freeS,ok:=cString(system);if !ok{return -1000};defer freeS()
	ct,freeT,ok:=cString(tools);if !ok{return -1000};defer freeT()
	var ci *C.char
	freeI:=func(){}
	if len(index)>0{ci,freeI,ok=cString(index);if !ok{return -1000};defer freeI()}
	return int(C.needle_init(cs,ct,ci))
}

func (a *cgoABI) Complete(text []byte,maxNewTokens int,output []byte)int{
	if len(output)==0{return -1000}
	ct,freeT,ok:=cString(text);if !ok{return -1000};defer freeT()
...
func nativeSupportError()error{return fmt.Errorf("native Needle engine requires linux/amd64 with CGO enabled")}
