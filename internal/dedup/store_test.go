...
func TestConcurrentDuplicatesExecuteOnce(t *testing.T) {
	...
	for i := range results {
		wg.Add(1)
		go func(i int) { defer wg.Done(); results[i] = store.Do(context.Background(), "start:3052", fn) }(i)
	}
	for calls.Load() != 1 {
	...
	close(start)
	wg.Wait()
	if calls.Load() != 1 || results[0].Value != "ok" || results[1].Value != "ok" || results[0].Deduplicated == results[1].Deduplicated {
...
func TestCachePolicyAndExpiry(t *testing.T) {
	...
	if store.Do(context.Background(), "k", fn).Value != 1 || store.Do(context.Background(), "k", fn).Value != 1 {
		t.Fatal("not cached")
	}
	now = now.Add(31 * time.Second)
	if store.Do(context.Background(), "k", fn).Value != 2 {
		t.Fatal("not expired")
	}
	noCache := func(context.Context) (any, CachePolicy, error) { calls++; return nil, DoNotCache, errors.New("known") }
	store.Do(context.Background(), "e", noCache)
	store.Do(context.Background(), "e", noCache)
	if calls != 4 {
